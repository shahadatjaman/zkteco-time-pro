...
import { deleteAUser, setUsers } from '@/store/slices/userSlice';
import { RootState } from '@/store/store';
import UpdateUserModal from '../create/UpdateUserModal';
...
const InfiniteTable: React.FC = () => {
  ...
  const { data: usersFromRootState } = useSelector((state: RootState) => state.user);
  const { data: users, isLoading, isError, isFetching, refetch, isSuccess } = useGetUsersQuery();
  ...
  const [deleteUser, { isLoading: isLoadingDele }] = useDeleteUserMutation();
  ...
  const onConfirmDel = async () => {
    if (deleteItem?._id) {
      const res: any = await deleteUser(deleteItem?._id);

      if (res && res.data && res.data.status === 200 && deleteItem._id) {
        dispatch(deleteAUser(deleteItem._id));
